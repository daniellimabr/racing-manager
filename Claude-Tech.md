...
**Atualizado 2026-07-21 (sessão 9 da trilha Racing, fim — ver Claude-Racing.md §2.21–2.26 pro detalhe completo):** dano/DNF/dificuldade recalibrados a partir de números concretos do PO (miss grave com chance de DNF instantâneo progressiva por saúde, todo tier desgasta saúde, dificuldade escala com saúde baixa, `healthMax` recalibrado); nitro renomeado contextualmente (KERS/Magic); playtest do próprio PO validou a sensação ("joguei quase 100% no purple, precisei de reparo rápido pra terminar — ótimo"). **Gate 1 tratado como provisório** (decisão datada na seção 7/3) — irmãos indisponíveis, soft-launch público descartado até haver um alpha/beta apresentável. PO vai tentar achar testers substitutos em paralelo (não bloqueia o resto).

1. **M2 destravado** — próxima frente de trabalho real é iniciar o detalhamento das tarefas de M2 (E-201 a E-206, seção 8), já que o Gate 1 provisório foi aceito pelo PO.
2. **Pendente de decisão de design (menor prioridade que M2, mas ainda em aberto):** a lacuna da ultrapassagem sem recompensa (Claude-Racing.md §2.24) — proposta do PO de custo-de-ritmo escalando com o gap ainda não implementada (só decidida a versão simplificada a se fazer primeiro, ver §2.25); `HEALTH_DIFFICULTY_FLOOR` e a tensão dos 2 critérios de `healthMax` (Claude-Racing.md §2.26) seguem sem confirmação final do PO.
3. **Item de conteúdo solto, não bloqueante:** exibir tempo de volta dos carros (pedido do PO, ainda não avaliado) — pode entrar em paralelo a qualquer momento, é aditivo.
4. Se/quando os testers substitutos (item paralelo do PO) trouxerem sinal, revisitar se isso muda alguma prioridade do M2 já em andamento.
