...
3. **Progressão de raridade — reformulada em 2026-07-21 (PO não entendeu a formulação original):** cada peça do carro sobe numa escada de 6 raridades (cinza→verde→azul→roxo→dourado→vermelho), subindo ao fundir 3 peças iguais em 1 melhor. O resultado do harness: mesmo um jogador que abre o app 5×/dia, todo dia, por 21 dias seguidos ("sem vida"), só chega em `purple` (o 4º degrau de 6) nesse período — os 2 últimos degraus (`gold`/`red`) ficariam pra bem depois disso (não simulei além de 21 dias; a curva sugere algo como 1-3 meses de jogo pra `gold` e vários meses pra `red`, mas isso é estimativa, não confirmado com simulação mais longa). **A pergunta real:** "o carro inteiro no topo (`red` em todos os 7 slots) levar vários meses de jogo, mesmo pra quem joga bastante todo dia" é o ritmo de longo prazo que você imagina pro jogo, ou você queria que o topo fosse alcançável mais rápido (semanas) ou ainda mais devagar/raro (mais prestigioso que isso)? Se quiser, rodo o harness com um período maior (ex.: 90 dias) numa próxima sessão pra confirmar a estimativa de `gold`/`red` com dado real em vez de extrapolação.
...
5. ~~**Auto-equipar vs. Oficina manual**~~ **— respondido pelo PO (2026-07-21): não, equipar deve ser escolha do jogador.** Auto-equipar é aceitável só como scaffolding temporário de teste até a Oficina (tela dedicada, CLAUDE.md §5) existir de verdade — não é uma solução permanente. A Oficina sobe de prioridade nos próximos passos (seção 5 abaixo), deixa de ser "só se o PO priorizar".
...
**Atualizado 2026-07-21 (resposta do PO):**

1. **Prioridade elevada — montar a tela de Oficina de verdade** (equipar peça manualmente, ver as 6 raridades por slot, modal de livery com 6 patrocinadores — CLAUDE.md tela 2): o PO rejeitou o auto-equipar como solução permanente (pergunta 5 respondida na seção 3) — o modelo de dados por trás (fusão/raridade/`zoneScale`) já existe e não muda, só falta a UI de escolha manual. Deixa de ser condicional a "se o PO priorizar".
2. **Bloqueado até a trilha Racing entregar:** a unificação do modelo de posição core/grid (pergunta 1, decisão do PO em Claude-Tech.md §3) — o cálculo de recompensa desta sessão continua provisório (usa a posição do grid) até isso acontecer.
3. Rodar o harness com um período mais longo (90 dias) pra confirmar a estimativa de `gold`/`red` citada na pergunta 3 (reformulada, resposta do PO ainda pendente).
4. Iniciar E-301 (Sede/escritórios) só depois de alguma resposta às perguntas 3/4, já que produção passiva + custos de escritório vão puxar a tabela de Gold pra outro ajuste.
