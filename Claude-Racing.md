...
> Última atualização: 2026-07-22 (sessão 15 — PO rodou uma nova run e pediu 3 coisas sem precisar de confirmação prévia: (1) zonas ainda mais estreitas — `ZONE_BASE_HALVES` cortado ~pela metade de novo (purple 1.5/green 10/amber 20), overshoot deliberado como nas sessões 10/13; cursor ~20-23% mais rápido (`RAMP_DURATION_MS`/`CURSOR_SWEEP_PERIOD_MS`), esse ajuste moderado (não overshoot), como pedido; (2) botão de pausa no HUD com modal "Continuar"/"Desistir da corrida" — desistir gera DNF instantâneo (`abandonRace()`, novo, motivo "desistência", sem penalidade de Gold nem revive); (3) animação da corrida deixou de teleportar/congelar entre eventos — carros avançam continuamente o tempo todo, desacelerando (bullet time) durante a decisão/desafio ativo em vez de parar, com um teto de segurança pra nunca ultrapassar visualmente um evento ainda não resolvido. 169/169 testes, build ok, smoke test Playwright manual cobrindo os 3 itens. Ver seção 2.33 pro detalhe técnico completo, incluindo uma limitação conhecida (pernas de perna curta "seguram" perto do teto) e uma tentativa inicial da animação que não funcionou e foi corrigida ainda na mesma sessão)
> Histórico de sessões anteriores (7 a 12) preservado no corpo do documento — ver a tabela de status (seção 1) e as subseções 2.20 a 2.30 pra o detalhe sessão a sessão. Header mantido só com a entrada mais recente pra não crescer indefinidamente.
...
| `HEALTH_DIFFICULTY_FLOOR` (0,6) | ⏳ **Superado pela sessão 13** — ver linha abaixo | Testado pelo PO: "não aumentou, nem de perto". Recalibrado pra 0,2 (sondagem agressiva). Ver seção 2.31 |
| "Roxo fácil demais" — geometria da zona | ⏳ **Superado pela sessão 13** — ver linha abaixo | 6 ainda era fácil ("vencendo tudo com facilidade"). Recalibrado pra 3 (sondagem agressiva). Ver seção 2.31 |
...
| Zona roxa / piso de saúde — sondagem agressiva | ⏳ **Calibração exagerada de propósito na sessão 13, aguardando o PO jogar de novo** | `ZONE_BASE_HALVES.purple` 6→3, `HEALTH_DIFFICULTY_FLOOR` 0,6→0,2. Não é a calibração final. Ver seção 2.31 |
| Bug de DNF mostrando posição "ao vivo" (P1 indevido) | ✅ **Corrigido na sessão 13** | `RaceOutput.position` força último lugar em DNF; bug relacionado de `state.position` desatualizado em eventos de saída também corrigido. Ver seção 2.31 |
| Boosts `rasante` e `fôlego de ultrapassagem` | ✅ **Implementados na sessão 13** | Últimos 2 dos 8 conceitos do CLAUDE.md §6.1 — todos implementados agora. Ver seção 2.31 |
| Débito técnico: `PIT_SCALE` declarada mas nunca usada | ✅ **Corrigido na sessão 13** | Número mágico duplicado em `timing.ts`, agora referencia a constante. Ver seção 2.31 |
| Pista de teste — Spa 8→4 voltas, pit na volta 3 | ✅ **Feito na sessão 14** | Config temporária pra ciclo de playtest mais rápido, pedido direto do PO. Ver seção 2.32 |
| Ultrapassagem — piso mínimo de dificuldade + bônus de ganho | ✅ **Feito na sessão 14** | `OVERTAKE_MIN_CLOSENESS`/`OVERTAKE_SUCCESS_GAIN_BONUS`. Ver seção 2.32 |
| Largada — cursor mais lento + zona própria mais estreita | ✅ **Feito na sessão 14** | `LARGADA_HOLD_RATE`/`FALL_RATE`/`LARGADA_ZONE_SCALE`. Ver seção 2.32 |
| `grid.ts` — companheiro configurável (`TeammateProfile`) | ✅ **Feito na sessão 14** | Camada aditiva, usada pelo E-302/E-303 (trilha Manager). Ver seção 2.32 |
| Zona/cursor — 2ª rodada de overshoot | ⏳ **Calibração exagerada de propósito na sessão 15, aguardando o PO jogar de novo** | `ZONE_BASE_HALVES` purple 3→1.5/green 20→10/amber 35→20; cursor 20-23% mais rápido. Ver seção 2.33 |
| Botão de pausa + desistência | ✅ **Feito na sessão 15** | `abandonRace()` (core) + `openPauseMenu`/`closePauseMenu` (view). Ver seção 2.33 |
| Animação contínua entre eventos (bullet time) | ✅ **Feito na sessão 15** | `visualPlayerT`/`advancePlayerRefT`; limitação conhecida em pernas muito curtas. Ver seção 2.33 |
...
### 2.31 Sessão 13 — calibração agressiva, bug de DNF, últimos 2 boosts, Sede (cross-ref), débito técnico

PO validou a calibração da sessão 10 jogando de verdade e trouxe 2 vereditos diretos, mais um bug real (posição no DNF) e um pedido de lista de tarefas ampla pras 3 trilhas + revisão de documentação. Resumo técnico (a rodada completa, incluindo Sede/escritórios que é trilha Manager, está registrada com mais detalhe em `Claude-Manager.md`):

**1. Calibração — modo sondagem, não ajuste fino:** PO relatou "ainda está fácil, tô vencendo tudo com facilidade" (zona roxa) e "com 50 de saúde [~23%] a dificuldade não aumentou, nem de perto" (piso de saúde), e pediu explicitamente "pode pegar pesado, pra eu validar o limite". `ZONE_BASE_HALVES.purple` 6→**3** (metade); `HEALTH_DIFFICULTY_FLOOR` 0,6→**0,2** (zona pode encolher até 80% com saúde baixa, contra 40% antes). Ambos deliberadamente exagerados — o objetivo é achar o limite superior de dificuldade jogando, não acertar de primeira. Nenhum dos dois é validável por bot (dependem de reflexo humano).

**2. Bug de DNF (P1 indevido), reportado pelo PO:** jogador crashou, encerrou a corrida, resumo mostrou P1. Causa raiz: o jogo não tinha regra de classificação de DNF — mostrava a posição "ao vivo" congelada no instante do abandono (os 11 carros de IA também param de avançar nesse ponto, a simulação não tem como "deixar o resto da corrida terminar sozinha"). Se o jogador tinha vantagem grande antes de bater, o crash sozinho podia não ser suficiente pra derrubar a posição na conta. Corrigido: `toRaceOutput()` força último lugar (`grid.cars.length + 1` = 12) sempre que `state.dnf === true`. **Bug relacionado, achado no processo:** `state.position` (campo cacheado) só era atualizado em eventos não-saída — como toda corrida que termina naturalmente termina numa saída, o campo ficava sempre 1 evento desatualizado no fim de QUALQUER corrida (não só DNF), afetando silenciosamente os números de posição do `tools/botHarness.ts` em todas as sessões de calibração anteriores. Corrigido: `state.position` sempre atualizado; a detecção de ultrapassagem (`positionChanged`, usada na mensagem/telemetria) continua restrita a eventos não-saída, sem mudança de comportamento aí — só o campo de posição em si deixou de ficar preso.

**3. Últimos 2 boosts (`rasante`, `fôlego de ultrapassagem`):** completam os 8 conceitos do CLAUDE.md §6.1. `rasante` (slipstream) é o único boost que se aplica na PRÓPRIA saída em que foi escolhido (não numa saída futura — a lógica de limpar `pendingBoost` em qualquer evento não-saída pelo meio inviabilizaria "guardar" o boost até uma saída mais adiante), +25% no ganho se já for positivo. `fôlego de ultrapassagem` alarga o limiar de gap pra tentar ultrapassar (`OVERTAKE_GAP_THRESHOLD` 1,0s → 1,5s) na próxima frenagem/pit — `canAttemptOvertake()` ganhou um parâmetro opcional `thresholdScale`.

**4. Débito técnico:** pequena auditoria (grep por TODO/FIXME, checagem de constantes não usadas) achou `PIT_SCALE` — declarada com um comentário claro, nunca importada em lugar nenhum; `computeScale()` (`timing.ts`) tinha o valor `1.3` duplicado como número mágico em vez de referenciar a constante. Corrigido. Também flagrado (não corrigível por código): o token de push do GitHub colado em texto puro no chat na sessão 3 segue sem confirmação de revogação — ver `Claude-Tech.md` §11.

**5. Sede do time (E-301) — trilha Manager, feita na mesma rodada:** 7 escritórios de produção passiva de peças, primeiro sink real de Gold do jogo. Detalhe completo em `Claude-Manager.md` (inclusive um bug real de `nowMs`/`Date.now()` achado no processo). Só sinalizando aqui pra quem ler só este documento não achar que a sessão foi só sobre Racing.

**6. 2ª pista real — avaliada e deliberadamente adiada:** dado o tempo já investido na sessão e a lição já registrada (§2.6: pra geometria real, buscar referência de verdade, nunca desenhar de memória), não havia uma imagem/SVG oficial de referência à mão pra fazer isso com qualidade. Fica pra uma sessão dedicada, usando a skill `track-layout` já existente.

**Verificação:** 120/120 testes (12 novos desta sessão: DNF+staleness, boosts novos, offices), `npx tsc --noEmit` limpo, `npm run build` ok. `npm run bots` rodado várias vezes ao longo da sessão — números de DNF/vitórias consistentes com a calibração da sessão 11 (a posição média subiu, mas isso é efeito esperado da correção do item 2, não regressão). Smoke tests Playwright headless (instalados/removidos, mesmo processo de sempre) em 2 rodadas separadas — tutorial + Sede navegando sem erro, HUD atualizando corretamente.

### 2.32 Sessão 14 — pista de teste mais curta, redesign de ultrapassagem/largada, companheiro configurável

PO confirmou 3 itens de feedback de um playtest anterior e pediu explicitamente uma pista mais curta pro ciclo de teste manual, depois autorizou "atacar a lista sem esperar minha interação" cobrindo também 3 épicos da trilha Manager (Pilotos/Carro 2, patrocinadores, Loja+Aura — ver `Claude-Manager.md`). Resumo do lado Racing:

**1. Pista de teste — Spa 8→4 voltas, pit na volta 3 (`tracks/spa.json`):** pedido direto ("a corrida muito longa demanda muito do meu tempo para validar o resultado"). Config **deliberadamente temporária** — `DAMAGE`/`healthMax`/demais constantes de calibração NÃO foram recalibradas em função disso. Efeito colateral esperado e aceito: DNF cai bastante no `botHarness` (menos exposição a eventos por corrida — Casual foi de ~27-34% pra ~3,6-5,4%), porque o alvo "real" de design continua sendo 8 voltas/~5min. Quando a pista voltar ao tamanho definitivo, os números de calibração devem ser reconferidos, não os desta config de teste.

**2. Ultrapassagem redesenhada** (`core/timing.ts` `computeScale`, `core/raceState.ts`, `core/constants.ts`): o PO pediu dificuldade proporcional à distância do carro da frente (mais longe = mais arriscado = mais difícil), mas que tentar nunca ficasse "de graça" mesmo com o carro colado, e que o bônus de sucesso nunca fizesse uma manobra perfeita perder tempo em relação a não tentar. Implementado com 2 mudanças:
   - `OVERTAKE_MIN_CLOSENESS = 0.2`: a fórmula de dificuldade (`scale *= 1 - 0.5 * closeness`) agora usa `closeness = OVERTAKE_MIN_CLOSENESS + (1 - OVERTAKE_MIN_CLOSENESS) * rawCloseness` em vez de `rawCloseness` puro — com gap≈0, `closeness` não cai a 0, fica em 0,2 (piso), então a penalidade mínima de dificuldade é sempre `1 - 0.5*0.2 = 0.9`, nunca "de graça".
   - `OVERTAKE_SUCCESS_GAIN_BONUS = 1.3`: em `raceState.ts`, se o evento não é saída/pit, o ganho é positivo E o jogador está tentando ultrapassagem, o `gain` recebe +30% ANTES do corte de 50% de saída (que não se aplica aqui, é frenagem) — assimétrico como o nitro: só multiplica ganho positivo, nunca agrava um resultado ruim, então nunca fica pior que não tentar.
   - Teste antigo ("gap próximo de 0 quase não penaliza") ficou falso com o piso novo — reescrito pra validar o piso em vez do comportamento antigo (`tests/timing.test.ts`).

**3. Largada redesenhada** (`view/viewConstants.ts`, `RaceScene.ts`): PO relatou o cursor rápido demais pra "segurar dentro do roxo" (largada usa o modelo de segurar contínuo do T-105, diferente do toque único de saída/frenagem normal). `LARGADA_HOLD_RATE` 0,16→0,10 e `LARGADA_FALL_RATE` 0,10→0,065 (cursor sobe/desce mais devagar). Como consequência pedida pelo PO ("sendo assim as zonas verde e roxa podem ser menores"), `LARGADA_ZONE_SCALE = 0.7` foi criado e aplicado SÓ na largada (`startTimingChallenge()` detecta `isLargada` e usa `scale * LARGADA_ZONE_SCALE` só nesse caso) — decisão deliberada de não tocar `ZONE_BASE_HALVES` compartilhado, que o PO tinha acabado de confirmar como correto pras outras zonas ("ficou mais difícil, na medida certa"). "Elástica" (pedido do PO) foi interpretado como só "mais lenta" nesta rodada, não física de mola de verdade — física real de elástico fica pra um revamp futuro do mecanismo inteiro (o próprio PO sinalizou que o desafio de timing "vai precisar de um revamp no futuro, especialmente quando ativarmos UI/FX").

**4. `core/grid.ts` — companheiro configurável (`TeammateProfile`):** camada aditiva pura — `createGridSim(rng, teammate?)` aceita um `TeammateProfile { weights, paceFactor }` opcional; sem ele, cai no perfil "Médio" fixo de sempre (nenhuma mudança de comportamento pra quem não usa). `createRace(track, setup, { teammate })` repassa pro grid. Existe pra dar suporte ao sistema de Pilotos da trilha Manager (E-302/E-303) — quem calcula o `TeammateProfile` a partir das skills de um piloto contratado é `core/pilots.ts`, que não é responsabilidade da trilha Racing (documentado com detalhe em `Claude-Manager.md`). Só sinalizando aqui pra quem ler só este documento entender de onde veio essa mudança em `grid.ts`.

**Verificação:** 169/169 testes (26 novos nesta sessão, mas a maioria é da trilha Manager — Racing contribuiu com os testes de ultrapassagem/largada em `tests/timing.test.ts`/`tests/raceState.test.ts`), `npx tsc --noEmit` limpo, `npm run build` ok, smoke test manual via Playwright (temp-install/uninstall, mesmo processo de sempre) navegando pelas 3 telas novas do Manager sem erro de console — não cobriu um replay completo de largada/ultrapassagem com clique de timing real (esses ficam validados pelos testes determinísticos + o próprio PO no próximo playtest).

### 2.33 Sessão 15 — calibração agressiva de zona/cursor, botão de pausa + desistência, animação contínua com bullet time

PO rodou uma nova run no build mais recente e trouxe 3 pedidos: zonas ainda fáceis demais, falta um botão de pausa com opção de desistir, e o visual de carros "parados" entre eventos parecendo "um jogo de trivia". Autorizou atacar os 3 sem pausar pra confirmação.

**1. Calibração agressiva (`core/constants.ts`, `view/viewConstants.ts`):** `ZONE_BASE_HALVES` cortado praticamente ao meio de novo (purple 3→1.5, green 20→10, amber 35→20) — mesmo espírito de overshoot deliberado das sessões 10/13 (ver [[feedback-calibration-overshoot]]), porque a rodada 13 (purple=3) já tinha sido testada pelo PO e seguia "muito fácil". `RAMP_DURATION_MS` 1300→1000 e `CURSOR_SWEEP_PERIOD_MS` 900→700 (cursor ~20-23% mais rápido) — ajuste moderado, não overshoot, porque o PO pediu explicitamente "um pouco mais rápido" (não "pega pesado" desta vez). Continua não validado pelo harness de bots (mesma ressalva de sempre: depende de reflexo humano real, não de sorteio por perfil).

**2. Botão de pausa + desistência (`RaceScene.ts`, `core/raceState.ts`):** botão "II" fixo no topo-centro do HUD (`buildPauseButton`), visível durante toda a corrida (some nas telas de DNF/resumo). Abre um overlay full-screen (`openPauseMenu`) com "Continuar"/"Desistir da corrida"; `this.time.paused = true` congela todos os `TimerEvent`/`delayedCall` da cena (timeout de desafio, prep, decisão de nitro/overtake), e os anchors baseados em `this.time.now` (`challengeStartTime`, `largadaGoAt`, `largadaLightsDoneAt` — que continuam correndo mesmo com `time.paused`, só os TimerEvents congelam) são compensados pelo tempo pausado em `closePauseMenu`, pra o cursor não "pular" ao despausar. Novo `abandonRace()` no core: DNF instantâneo com `dnfReason = 'desistência'`, sem penalidade de Gold nem oferta de revive (decisão deliberada do jogador, não falha de pilotagem) — direto pro resumo (`showSummary(true)`).

**3. Animação contínua com bullet time (`RaceScene.ts`, `view/viewConstants.ts`):** tentativa inicial (perseguir a posição do evento atual com suavização exponencial, só variando a velocidade por `bulletTime`) **não resolvia o problema de verdade** — o alvo fica parado até `advance()` rodar, e a fase "rápida" já alcançava esse alvo antes do desafio nem começar, então o carro chegava e ficava parado do mesmo jeito (confirmado com Playwright: ícones pixel-idênticos em screenshots 500ms+ à parte durante um passo de decisão). Corrigido com um modelo diferente: `RaceScene.visualPlayerT` avança continuamente em direção ao **próximo** evento (`state.events[eventIndex+1]`, espiado com antecedência — a sequência é pré-computada), nunca em direção ao atual — nunca fica parado enquanto o desafio não resolve, só desacelera (`bulletTime`, ligado em `showPreChallenge`, desligado no início de `resolveChallenge` — cobre a cadeia inteira de decisão: overtake, nitro, prep, desafio ativo). `LEG_PROGRESS_CAP` (0,95) trava o avanço perto da marca do próximo evento sem chegar/passar antes de `advance()` commitar, evitando que uma cadeia de decisões muito longa (múltiplos timeouts em sequência) "ultrapasse" visualmente curvas ainda não resolvidas. Os outros 11 carros do grid continuam posicionados por deslocamento de gap em relação a essa mesma referência contínua — o pelotão inteiro anda junto.
   - **Limitação conhecida, aceita por ora:** pernas muito curtas (ex.: frenagem→saída da mesma curva, ou largada→frenagem da 1ª curva quando os 2 eventos caem no mesmo `pathIndex`, caso de La Source em Spa) atingem o teto de avanço em poucos segundos e então "seguram" ali até a resolução — mesma limitação que qualquer modelo com teto de segurança teria. Verificado via Playwright que pernas normais (corner-to-corner, a maioria da pista) mostram avanço claro e contínuo ao longo de vários segundos de decisão (comparação de screenshots em t=20s/22s durante o mesmo desafio ativo). Revisitar se o PO notar o "segurar" nas pernas curtas específicas.

**Verificação:** 169/169 testes, `npm run build` ok, smoke test manual via Playwright (temp-resolve do pacote cacheado por `npx`, sem alterar `package.json`) cobrindo: pausa abre/bloqueia clique por baixo (overlay absorve o toque)/resume sem perder o estado do desafio em andamento/desistência gera DNF com motivo correto e sem penalidade de Gold/botão some nas telas de fim; largada com zona visivelmente mais estreita; carros avançando de fato durante um desafio ativo (aceleração da largada) e ao longo de uma leitura+decisão de vários segundos numa perna longa.
...
- ~~**Sugestão de conteúdo do PO: exibir tempo de volta dos carros**~~ — **feito na sessão 12**, ver §2.30.
...
- ~~**`HEALTH_DIFFICULTY_FLOOR = 0,6` ainda não confirmado pelo PO**~~ — **testado na sessão 13:** o PO confirmou que não sentia diferença perceptível. Recalibrado pra 0,2 (sondagem agressiva, não é a calibração final) — ver §2.31 e a pendência mais recente sobre isso na tabela de status (seção 1).
...
- ~~**DNF mostra a posição "ao vivo" de quando o jogador parou, não uma classificação de "não terminou" (P1 indevido)**~~ — **corrigido na sessão 13, ver §2.31.** `RaceOutput.position` força último lugar em DNF; o bug relacionado de `state.position` desatualizado em eventos de saída (afetava os números do harness em toda calibração anterior) também foi corrigido.
- **Ultrapassagem redesenhada (sessão 14, §2.32) e largada mais lenta (sessão 14, §2.32) ainda não confirmadas em playtest humano real.** Implementadas e verificadas tecnicamente (testes/tsc/build/smoke), mas o piso de dificuldade, o bônus de 30% e o novo ritmo do cursor de largada são todos parâmetros de "feel", que o harness de bots não consegue validar (roda por probabilidade de tier, não timing físico). Aguardando o próximo playtest do PO.
- **Pista de Spa em 4 voltas é temporária (sessão 14, §2.32) — não recalibrar `DAMAGE`/`healthMax` em torno dela.** Quando a pista voltar a 8 voltas (tamanho de design definitivo), reconferir os números de DNF do `botHarness`, que caíram bastante só por causa da menor exposição a eventos, não por causa de nenhuma mudança de dificuldade real.
