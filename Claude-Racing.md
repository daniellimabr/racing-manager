...
> Última atualização: 2026-07-21 (sessão 14 — PO autorizou avançar sem interrupção sobre uma lista combinada. Resumo do lado Racing: (1) Spa reduzido de 8 pra **4 voltas, pit na volta 3** — pedido explícito pra encurtar o ciclo de teste manual (config temporária, DAMAGE/healthMax não recalibrados de propósito); (2) ultrapassagem redesenhada — piso mínimo de dificuldade extra (`OVERTAKE_MIN_CLOSENESS`, nunca fica "de graça" nem com gap≈0) + bônus de ganho no sucesso (`OVERTAKE_SUCCESS_GAIN_BONUS`, assimétrico como o nitro — só ajuda, nunca piora); (3) largada com cursor mais lento (`LARGADA_HOLD_RATE`/`FALL_RATE`) + zona própria mais estreita (`LARGADA_ZONE_SCALE`, sem tocar a zona compartilhada de frenagem/aceleração/pit); (4) `core/grid.ts` ganhou `TeammateProfile` opcional — o companheiro de equipe (Carro 2) agora pode ser configurado por fora em vez do perfil "Médio" fixo, usado pela trilha Manager pro sistema de Pilotos (E-302/E-303, detalhe completo em `Claude-Manager.md`). 169/169 testes, tsc limpo, build ok, smoke test Playwright manual sem erros de console. Ver seção 2.32 pro detalhe técnico completo)
