...
| T-110, playtest sessões 1 e 2 (PO sozinho) | ⏳ **2 de 3 sessões feitas (sessão 8)** | Faltam os 2 irmãos pro Gate 1 completo. Ver seções 2.21/2.22 |
...
| Dificuldade (roxo fácil) / ultrapassagem sem recompensa | ⏳ **Causa raiz encontrada na sessão 9, decisão de design pendente do PO/CTO** | Ver seção 2.24 |
...
- **Dificuldade fácil demais (roxo) — confirmado por playtest humano E por harness, mas ainda sem correção (sessão 9, §2.24).** DNF continua 0% em todos os perfis do harness. A causa da dificuldade física de acertar roxo (`ZONE_BASE_HALVES`/`RAMP_DURATION_MS`) é invisível pros bots (eles simulam por sorteio de probabilidade, não por geometria de zona) — só um playtest humano pode validar uma mudança aqui. **Decisão pendente do PO/CTO:** proponho testar um valor de `ZONE_BASE_HALVES.purple` reduzido (ex.: 8 → 6) no próximo playtest e comparar a sensação, em vez de eu adivinhar um número sem forma de verificar.
- **Mecânica de ultrapassagem: causa raiz encontrada (sessão 9, §2.24), é lacuna de design, não bug.** Tentar ultrapassagem só estreita a zona (risco), sem nenhum bônus de `GAIN`/chance de troca de posição (a mudança de posição já acontece "de graça" com um resultado roxo, com ou sem a flag marcada) — bate com o texto literal do CLAUDE.md, que só descreve o risco, nunca prometeu recompensa extra. **Decisão pendente do PO/CTO:** vale dar um bônus explícito de `GAIN` (ou de chance de troca de posição) quando `overtakeAttempt` + resultado bom, pra a decisão ter upside real, não só risco? Não implementado — muda o significado da mecânica, não é ajuste de número.
...
- **`HEALTH_DIFFICULTY_FLOOR = 0,6` não confirmado pelo PO (sessão 9, §2.26)** — implementado como proposta (§2.25), mas o PO não respondeu esse parâmetro especificamente na última mensagem, só os valores de `DAMAGE`/filosofia de `healthMax`. Válido revisar depois do próximo playtest.
- **Tensão não resolvida entre os 2 critérios de calibração de `healthMax` (sessão 9, §2.26).** "Verde à toa = metade da saúde" pede `healthMax≈219`; DNF razoável pra Casual/Temerário pede muito mais (280-320+) por causa do piso criado pelo DNF instantâneo no miss, que é independente de `healthMax`. Fixei 260 como meio-termo, mas o PO pode preferir mexer em `MISS_INSTANT_DNF_CHANCE_MIN/MAX` em vez de `healthMax` pra resolver isso — não decidido, fica pra próxima rodada.
- **`tests/grid.test.ts` tem 1 teste flaky (RNG interno sem seed)** — achado incidental na sessão 9, não corrigido (fora do escopo pedido), não é regressão desta sessão.
