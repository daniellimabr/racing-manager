...
- **`tests/grid.test.ts` tem 1 teste flaky (RNG interno sem seed)** — achado incidental na sessão 9, não corrigido (fora do escopo pedido), não é regressão desta sessão. Ainda não corrigido na sessão 10 (fora do escopo desta rodada também).
...
5. Se o Manager (M2) for consumir `RaceOutput.position`, revisitar a divergência entre posição do core e posição do grid (seção 3) — confirmada em jogo real nesta sessão (gap/ultrapassagem incoerentes quando o grid já promove o jogador a 1º antes do core). **Atualização 2026-07-21 (pós-sessão 10, resposta do PO aos relatórios de calibração):** o Manager (sessão 1 da trilha, em paralelo) bateu exatamente nesse problema pra decidir a posição usada na recompensa Gold/peças, e o PO respondeu rejeitando "escolher um lado" — quer os 2 modelos unificados de vez, não mais um patch por sintoma (já foram 2: o líder recebendo oferta de ultrapassagem nesta seção, e agora a recompensa do Manager). **Vira a prioridade #1 da próxima sessão desta trilha** (decisão datada em Claude-Tech.md §3). Esboço de direção técnica pra próxima sessão avaliar (não é decisão fechada, só ponto de partida): o `grid.ts` já simula os 12 carros com pace individual pra HUD — provável candidato a virar a única fonte de verdade, com o jogador entrando nele como mais um carro simulado a partir do seu próprio `raceProgress`; `raceState.position`/`gapToAhead` passariam a ser *derivados* do grid (não recalculados em paralelo). Precisa revisar com cuidado os ~56 testes de `raceState.test.ts` que dependem do modelo escalar atual (gap/ultrapassagem/DNF), já que é uma mudança de arquitetura, não um ajuste de constante.
