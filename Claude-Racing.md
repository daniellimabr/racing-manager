...
> Última atualização: 2026-07-21 (sessão 7, feedback de playtest do PO — gap do HUD passou a ser sempre relativo ao carro da frente, não ao líder; corrigido bug em que o líder recebia oferta de ultrapassagem (divergência core/grid da seção 3 se materializando de novo); carros agora se movem visivelmente após a frenagem E após a aceleração, não só numa das duas transições; `git` (nunca tinha sido de fato posto no PATH, só contornado sessão a sessão) corrigido de vez via registro — ver seção 2.20)
> Última atualização anterior: 2026-07-21 (sessão 6, rodada autônoma de ~2h sem check-in, autorizada pelo PO — causa raiz real do prompt de permissão sem "sempre permitir" encontrada e corrigida (não era só cache de settings, ver seção 2.19); traçado de Spa refeito pela 3ª vez, desta vez com matemática exata sobre o SVG oficial em vez de "a olho", e validado por um agente independente; skill `track-layout` + agente `track-layout-validator` criados para isso ficar repetível; UX do nitro trocada de toggle+confirmar pra 2 botões diretos; boost "pneu" renomeado de novo pra "Bono, My Tyres"; HUD ganhou painel de gap-ao-líder de todos os 12 pilotos; Node encontrado (instalação portátil solta em `C:\node-v24.15.0-win-x64`, nunca estava no PATH) e adicionado ao PATH — `npm test`/`tsc`/`build` rodaram e passaram no final da sessão, ver seção 2.19)
