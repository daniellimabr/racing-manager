...
> Última atualização: 2026-07-23 (rodada 13 — 2 bugs de pilotagem corrigidos a pedido do PO, nenhuma decisão de design nova, ambas as pistas: (1) **boost oferecido no lugar errado** — aparecia na SAÍDA da última curva da volta anterior, ou seja, ainda antes de cruzar a linha de largada/chegada (e, em voltas com pit, antes até do próprio pit); movido pra SAÍDA da 1ª curva de cada volta seguinte (precisa continuar sendo uma saída, não uma frenagem, porque o boost "rasante" só funciona se aplicado na própria saída em que foi escolhido — `core/track.ts`); (2) **carros "teleportando" pra próxima curva depois da aceleração, sem animação até o bullet time do desafio seguinte** — causa raiz no crawl visual contínuo (`RaceScene.advancePlayerRefT`): ele só sabia se aproximar do evento ATUAL sendo desafiado; quando `advance()` trocava esse alvo pra um evento bem mais à frente (fim de uma reta inteira), o cálculo de progresso zerava e o ícone reaparecia direto na nova posição no mesmo frame, em vez de percorrer a distância. Corrigido com uma fase de "viagem" explícita (interpolação de tempo fixo, reaproveitando o mesmo orçamento que já existia como pausa "morta" — não mudou o ritmo total da corrida): o próximo desafio (e o bullet time dele) só começa depois que essa viagem termina. Detalhe técnico completo em Claude-Racing.md §2.38. Falta playtest humano real dos 2 fixes — comportamento visual do Phaser não é coberto pelos testes automatizados.)
> Última atualização anterior: 2026-07-23 (rodada 12 — 3 bugs de pilotagem/pista corrigidos a pedido do PO, nenhuma decisão de design nova: (1) **carros se moviam visualmente antes da largada de verdade**, durante a contagem 3-2-1-JÁ — causa raiz no loop `update()` do `RaceScene`, que avançava o crawl contínuo dos carros mesmo antes do 1º evento (largada) começar a ser mostrado; isso também explicava os outros 2 sintomas relatados (virada de volta parecendo cair no lugar errado, nome da curva no HUD não batendo com a posição visual do carro); (2) **linha de largada/chegada de Interlagos** estava no ponto errado do traçado — corrigida por ROTAÇÃO do path já validado (a forma não mudou, só qual ponto é o índice 0), pra ficar logo antes da curva "S do Senna", confirmado por um SVG de referência oficial fornecido pelo PO; (3) **Spa redesenhada** a partir do parsing numérico direto (não de memória) do SVG de referência oficial (Wikipedia), corrigindo os ângulos agudos que não existiam na pista real — mesma curadoria de 9 curvas de antes (Q3 já registrava essa regra), só a geometria mudou. Ambas as pistas validadas por um agente independente (`track-layout-validator`) antes de aplicar, seguindo o protocolo do skill `track-layout` (nunca redesenhar circuito real de memória — CLAUDE.md §2.6 do Claude-Racing.md já registra 2 tentativas anteriores de Spa rejeitadas por esse motivo). Detalhe técnico completo em Claude-Racing.md §2.37. Falta playtest humano real do fix de "carros antes da largada" — comportamento visual do Phaser não é coberto pelos testes automatizados.)
## 1. Visão
...
| 2026-07-22 | **2ª pista (Interlagos) e 1º campeonato (2 corridas) entregues:** Interlagos (layout 1990, 13 curvas nomeadas) criada a partir de SVG de referência real, validada geometricamente por agente independente. Campeonato de construtores Spa→Interlagos, pontuação clássica de F1, reaproveitando a infra de grid/`teamId` já existente (execução direta da Q4, já decidida). Detalhe técnico em Claude-Racing.md §2.36 / Claude-Manager.md §2.13 |
...
8. **Seleção de campeonato/corrida** ✅ implementada parcialmente (`ChampionshipScene`, 2026-07-22) — campeonato de construtores fixo de 2 corridas (Spa→Interlagos), calendário + standings + botão "Correr". Progressão completa Kart/Turismo/Fórmula com regional/continental (Q11) ainda não implementada — isso é o próximo passo natural quando houver mais de 1 campeonato/categoria
9. **Corrida (gameplay)** ✅ implementada (`RaceScene`) — pista selecionável (Spa ou Interlagos, registry `TRACKS`, 2026-07-22), mapa do circuito em retrato, ícones escalonados (piloto 1 maior, piloto 2 destacado, pódio destacado, demais pequenos), HUD com barra de timing (zonas vermelha/amarela/verde/roxa), botão acelerar/frear, toggles persistentes de ultrapassagem/nitro, indicador de saúde do carro, tempo de volta, botão de pausa
